...
#ifndef GRAPH_PARTITIONER_SHARDING_CONSTRAINT_HPP
#define GRAPH_PARTITIONER_SHARDING_CONSTRAINT_HPP

#include "graph_basic_types.hpp"
#include "generate_pds.hpp"
#include <algorithm>
#include <vector>
#include <cmath>
   ...
   public:
    sharding_constraint(size_t num_shards, std::string method) {
      ...
      if (method == "grid") {
        ...
        std::cerr << "Unknown sharding constraint method: " << method << std::endl;
      }
   ...
   private:
    void make_grid_constraint() {
      ...
      if (!is_grid_compatible(nshards, nrows, ncols)) {
        std::cerr << "Num shards: " << nshards << " cannot be used for grid ingress." << std::endl;
      };
    ...
    void make_pds_constraint() {
      ...
      if (!is_pds_compatible(nshards, p)) {
        std::cerr << "Num shards: " << nshards << " cannot be used for pdsingress." << std::endl;
      };
      pds pds_generator;
